...
class MainForm:
  ...
  def open_file(self, event):
    ...
    if self.response == gtk.RESPONSE_CANCEL:
      ...
      self.chooser.destroy()
      self.chooser.destroy()
      self.hightlighting(0,event)
      
  def hightlighting(self, widget, event):
    text = buff.get_text(buff.get_start_iter() , buff.get_end_iter())
    search = ["function", '/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+/']
    searchCol = ["#0404B4", "#088A08"]
    ind = 0
    buff.set_text("")
    lessWord = ""
    lessInd = 0
	...
	ind = -1
      ...
      if (ind <> -1):
	#buff.put_text(text[0:ind], '#000000', '#FFFFFF', 'monospace', 9)
	buff.put_text(lessWord, searchCol[index], '#FFFFFF', 'monospace', 9)
	text = text[ind + len(lessWord):]
	lessWord = ""
    
    #buff.put_text(text, '#000000', '#FFFFFF', 'monospace', 9)
